...
void content_set_status(struct content *c, const char *status_message, ...)
{
	va_list ap;
	int len;
LOG(("content %s (%p) status %s", c->url, c, status_message));
	...
	va_end(ap);
LOG(("content %s (%p) status %s", c->url, c, c->sub_status));
...
void content_convert(struct content *c, int width, int height)
{
	...
	assert(c);
	LOG(("content %s (%p) convert to %i x %i",
			c->url, c, width, height));
	assert(c->type < HANDLER_MAP_COUNT);
	LOG(("content %s (%p) type %s", c->url, c,
			content_type_name[c->type]));
	assert(c->status == CONTENT_STATUS_LOADING);
	LOG(("content %s (%p) status %s",
			c->url, c, content_status_name[c->status]));
	assert(!c->locked);
	...
	LOG(("content %s (%p) converted to %s",
			c->url, c, content_type_name[c->type]));
		...
		intptr_t p1, intptr_t p2)
{
    ...
    if (!callback || (unsigned int)callback < 0x80000 || (unsigned int)callback > 0x20000000) {
        LOG(("ERROR: Rejecting invalid callback 0x%08x", (unsigned int)callback));
        return false;
    }
	
	LOG(("content_add_user %s (%p), user %p 0x%" PRIxPTR " 0x%" PRIxPTR,
...
{
	struct content_user *user, *next;
	LOG(("Before assert %p %s", c, c->url));
	assert(c);
	LOG(("AFter assert %p %s", c, c->url));	
        ...
        union content_msg_data data)
{
    ...
    LOG(("XXXXX NOWA WERSJA - Before assert %p %s", c, c->url));
    assert(c);
    LOG(("XXXXX NOWA WERSJA - After assert %p %s", c, c->url));
    
    // Sprawdź czy user_list istnieje
    if (!c->user_list) {
        LOG(("ERROR: user_list is NULL for content %p", c));
        return;
    }
    
    LOG(("user_list=%p, user_list->next=%p", c->user_list, c->user_list->next));
    
    // USUNIĘTO WSZYSTKIE WALIDACJE - test czy lista działa
    for (user = c->user_list->next; user != 0 && count < 100; user = next) {
        count++;
        
        LOG(("Processing user %d: %p, callback=%p, p1=%p, p2=%p", count, user, user->callback, user->p1, user->p2));
        
        next = user->next;  /* user may be destroyed during callback */
        LOG(("Next user will be: %p", next));
        
        if (user->callback != 0) {
            LOG(("Calling callback for user %p", user));
            LOG(("Callback address: 0x%08x, p1=0x%08x, p2=0x%08x", 
                (unsigned int)user->callback, (unsigned int)user->p1, (unsigned int)user->p2));
            
            user->callback(msg, c, user->p1, user->p2, data);
            LOG(("Callback completed for user %p", user));
        }
        
        // Sprawdź czy next nie wskazuje na już przetworzony element
        if (next == c->user_list->next) {
            LOG(("LOOP DETECTED: next points to first element again"));
            break;
        }
    }
    
    if (count >= 100) {
        LOG(("WARNING: Stopped iteration after %d users (possible infinite loop)", count));
    }
    
    LOG(("content_broadcast completed, processed %d users", count));
}
#endif
