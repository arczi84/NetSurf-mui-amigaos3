...
STATIC BOOL create_offscreen_bitmap(APTR obj, struct Data *data, ULONG width, ULONG height)
{
    ...
    if (CyberGfxBase && depth > 8) {
        ...
        bytes_per_row = (((width + 15) >> 4) << 1) * depth;
    }
    ULONG bytes_needed = bytes_per_row * height + 1024; // Overhead

    ULONG avail_fast = AvailMem(MEMF_FAST | MEMF_LARGEST);
    ULONG avail_chip = AvailMem(MEMF_CHIP | MEMF_LARGEST);
    ULONG avail = (CyberGfxBase && depth > 8) ? avail_fast : avail_chip;

    if (avail < bytes_needed * 1.5) { // 50% buffer
        LOG(("WARNING: Insufficient memory (%lu available, %lu needed). Reducing size.", avail, bytes_needed));
        width = MIN(width, 800);
        height = MIN(height, 600);
        // Recalculate bytes_needed if reduced
        if (CyberGfxBase && depth > 8) {
            bytes_per_row = width * (depth / 8);
        } else {
            bytes_per_row = (((width + 15) >> 4) << 1) * depth;
        }
        bytes_needed = bytes_per_row * height + 1024;
    }
    ...
    if (GetBitMapAttr(data->BitMap, BMA_FLAGS) & BMF_STANDARD) {
...
STATIC VOID render_content_to_bitmap(APTR obj, struct Data *data)
{
    ...
    float scale = data->context->bw->scale;
    ULONG bg = 0xFFFFFF; // White background
    ...
    renderinfo.origin_x = 0;
    renderinfo.origin_y = 0;

    current_redraw_browser = data->context->bw;
    plot = muiplot;

    // Clear the bitmap with background color
    SetColorAB(data->RastPort, bg, 0);
    RectFill(data->RastPort, 0, 0, data->bm_width - 1, data->bm_height - 1);

    // Render the visible portion with offset
    content_redraw(c, -vleft, -vtop, data->bm_width, data->bm_height,
                   0, 0, data->bm_width, data->bm_height,
                   scale, bg);

    // If content is smaller than bitmap, fill remaining areas
    LONG content_draw_width = (LONG)(c->width * scale);
    LONG content_draw_height = (LONG)(c->height * scale);

    if (content_draw_width < data->bm_width) {
        SetColorAB(data->RastPort, bg, 0);
        RectFill(data->RastPort, content_draw_width, 0,
                 data->bm_width - 1, data->bm_height - 1);
    }
    if (content_draw_height < data->bm_height) {
        SetColorAB(data->RastPort, bg, 0);
        RectFill(data->RastPort, 0, content_draw_height,
                 data->bm_width - 1, data->bm_height - 1);
    }
...
DEFMMETHOD(Setup2)
{
    ...
    if (rc) {
        ...
        struct ColorMap *cm = _screen(obj)->ViewPort.ColorMap;
        if (global_pen_a < 0) {
            global_pen_a = ObtainPen(cm, -1, 0, 0, 0, PENF_EXCLUSIVE);
            if (global_pen_a >= 0) LOG(("DEBUG: Pen A allocated: %ld", global_pen_a));
            else LOG(("ERROR: ObtainPen A failed - check memory or colormap"));
        }
        if (global_pen_b < 0) {
            global_pen_b = ObtainPen(cm, 0, 0, 0, -1, PENF_EXCLUSIVE);
            if (global_pen_b >= 0) LOG(("DEBUG: Pen B allocated: %ld", global_pen_b));
            else LOG(("ERROR: ObtainPen B failed - check memory or colormap"));
...
DEFMMETHOD(Draw)
{
    ...
    if (msg->flags & (MADF_DRAWOBJECT | MADF_DRAWUPDATE)) {
        ...
        if (data->vleft_old != vleft || data->vtop_old != vtop || data->changed || data->redraw_pending || !data->BitMap) {
            ...
            // Temporary fill with white
            struct RastPort *rp = _rp(obj);
            ULONG depth = GetBitMapAttr(rp->BitMap, BMA_DEPTH);
            if (CyberGfxBase && depth > 8) {
                FillPixelArray(rp, _mleft(obj), _mtop(obj), data->mwidth, data->mheight, 0xFFFFFFFF);
            } else {
                SetColorAB(rp, 0xFFFFFF, 0);
                RectFill(rp, _mleft(obj), _mtop(obj), _mleft(obj) + data->mwidth - 1, _mtop(obj) + data->mheight - 1);
            }
        } else {
