...
static bool global_history_add_internal(const char *url, const struct url_data *data);

/**
 * Create and initialise a node
 */
static void mui_global_history_initialise_node(const char *title,
 ...
 */
static void mui_global_history_initialise_nodes(void)
{
	struct tm *full_time;
	time_t t;
	int weekday;
	...
	mui_global_history_initialise_node((char *)messages_get("DateToday"), t, 0);
	if (weekday > 0)
		mui_global_history_initialise_node(
				(char *)messages_get("DateYesterday"), t, -1);
	for (i = 2; i <= weekday; i++)
		mui_global_history_initialise_node(NULL, t, -i);
	mui_global_history_initialise_node((char *)messages_get("Date1Week"),
				t, -weekday - 7);
	mui_global_history_initialise_node((char *)messages_get("Date2Week"),
				t, -weekday - 14);
	mui_global_history_initialise_node((char *)messages_get("Date3Week"),
				t, -weekday - 21);
}
 ...
 */
static struct node *mui_global_history_find(const char *url)
{
	int i;
	struct node *node;
...
void mui_global_history_initialise(void)
{
	...
	mui_global_history_initialise_nodes();
		...
		const struct url_data *data)
{
	...
	if (parent->deleted) {
		...
		tree_set_node_selected(&global_history_tree, parent, false);
		tree_set_node_expanded(&global_history_tree, parent, false);
		tree_link_node(link, parent, before);
/*
		if (!global_history_init) {
		  	tree_recalculate_node(&global_history_tree, parent, true);
		  	tree_recalculate_node_positions(&global_history_tree, global_history_tree.root);
...
*/
	...
	if (!global_history_init) {
		node = mui_global_history_find(url);
		if (node) {
...
*/
			...
			tree_handle_node_changed(&global_history_tree, node, false, true);
			return true;
		}
	}
...
/*
		tree_redraw_area(&global_history_tree,
				...
				0, NODE_INSTEP, 16384);
*/
		tree_handle_node_changed(&global_history_tree, node, true, false);
	}
