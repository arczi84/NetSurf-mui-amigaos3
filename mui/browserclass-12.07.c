...
STATIC VOID delete_offscreen_bitmap(struct Data *data)
{
    ...
    if (data->Layer) {
        // Sprawdź czy TTEngine jest dostępne
        if (TTEngineBase) {
...
STATIC BOOL create_offscreen_bitmap1(APTR obj, struct Data *data, ULONG width, ULONG height)
{
    ...
    // Sprawdź głębokość ekranu dla RTG
    ULONG depth = GetBitMapAttr(screen_bm, BMA_DEPTH);
    if (depth < 8) {
        LOG(("WARNING: Screen depth too low for RTG: %lu", depth));
        // Można dodać fallback na natywny chipset
    }

    // Ogranicz rozmiar bitmap do rozumnych limitów dla RTG
    data->bm_width = MIN(width, 1024);   // Max szerokość
    data->bm_height = MIN(height, 768);  // Max wysokość
    
    // Sprawdź dostępną pamięć przed alokacją
    ULONG bytes_needed = ((data->bm_width + 15) / 16) * 2 * data->bm_height * depth;
    if (AvailMem( MEMF_FAST) < bytes_needed * 2) {
        LOG(("WARNING: Insufficient memory for bitmap: %lu bytes needed", bytes_needed));
        // Zmniejsz rozmiar lub użyj window RastPort
        data->bm_width = MIN(data->bm_width, 640);
        data->bm_height = MIN(data->bm_height, 480);
    }
    ...
    data->BitMap = AllocBitMap(data->bm_width, data->bm_height, depth, BITMAP_FLAGS, screen_bm);
    if (!data->BitMap) {
        LOG(("ERROR: Failed to allocate BitMap"));
        return FALSE;
    }

    // Dodaj sprawdzenie czy bitmap został faktycznie przydzielony w odpowiedniej pamięci
    if (GetBitMapAttr(data->BitMap, BMA_FLAGS) & BMF_STANDARD) {
        LOG(("INFO: Using standard bitmap (not RTG)"));
    }
    ...
    LOG(("DEBUG: RTG bitmap created successfully: %lux%lux%lu", 
         data->bm_width, data->bm_height, depth));
...
DEFMMETHOD(Setup)
{
    ...
    if (rc) {
        LOG(("DEBUG: Setup called, checking RTG capabilities"));
        
        // Sprawdź dostępność CyberGraphX
        if (CyberGfxBase) {
            LOG(("DEBUG: CyberGraphX available"));
        } else {
            LOG(("WARNING: CyberGraphX not available, using native chipset"));
        }

        // Sprawdź dostępność TTEngine
        if (TTEngineBase) {
            LOG(("DEBUG: TTEngine available"));
        } else {
            LOG(("WARNING: TTEngine not available"));
        }
