...
{
	LOG(("browser_window_create: url %s, clone %p, referer %s",
		url ? url : "NULL", clone, referer ? referer : "NULL"));
	...
	LOG(("browser_window_create: allocated bw %p", bw));
	/* Initialise common parts */
	browser_window_initialise_common(bw, clone);
	LOG(("browser_window_create: common parts initialised"));
		...
		intptr_t p1, intptr_t p2, union content_msg_data data)
{
	...
	LOG(("DEBUG: browser_window_callback: bw=%p, msg=%d, content=%p, p1=%p, p2=%p",
		bw, msg, c, (void*)p1, (void*)p2));
	
	// Wyprintuj nazwę komunikatu
	...
	if (msg >= 0 && msg < (sizeof(msg_names)/sizeof(msg_names[0]))) {
		LOG(("DEBUG: Message: %s", msg_names[msg]));
	}

	LOG(("browser_window_callback: bw %p, msg %d, content %p", bw, msg, c));
	
	switch (msg) {
	case CONTENT_MSG_LOADING:
		LOG(("DEBUG: Processing CONTENT_MSG_LOADING"));
	...
	case CONTENT_MSG_READY:
		//kprintf("DEBUG: Processing CONTENT_MSG_READY\n");
		assert(bw->loading_content == c);
		...
		//kprintf("DEBUG: Calling browser_window_update(bw, true)\n");
	...
	case CONTENT_MSG_DONE:
		//kprintf("DEBUG: Processing CONTENT_MSG_DONE\n");
		assert(bw->current_content == c);

		//kprintf("DEBUG: Calling browser_window_update(bw, false)\n");
		browser_window_update(bw, false);
		browser_window_set_status(bw, c->status_message);
		browser_window_stop_throbber(bw);
	...
	case CONTENT_MSG_ERROR:
		//kprintf("DEBUG: Processing CONTENT_MSG_ERROR: %s\n", data.error);
	...
	case CONTENT_MSG_STATUS:
		//kprintf("DEBUG: Processing CONTENT_MSG_STATUS\n");
		browser_window_set_status(bw, c->status_message);
		break;

	case CONTENT_MSG_REFORMAT:
		//kprintf("DEBUG: Processing CONTENT_MSG_REFORMAT\n");
		...
		//kprintf("DEBUG: Calling browser_window_update(bw, false) from REFORMAT\n");
		browser_window_update(bw, false);
		break;

	case CONTENT_MSG_REDRAW:
		//kprintf("DEBUG: Processing CONTENT_MSG_REDRAW - calling gui_window_update_box\n");
		gui_window_update_box(bw->window, &data);
		break;

	case CONTENT_MSG_NEWPTR:
		//kprintf("DEBUG: Processing CONTENT_MSG_NEWPTR\n");
		bw->loading_content = c;
		if (data.new_url) {
	...
	case CONTENT_MSG_LAUNCH:
		//kprintf("DEBUG: Processing CONTENT_MSG_LAUNCH\n");
	...
	case CONTENT_MSG_AUTH:
		////kprintf("DEBUG: Processing CONTENT_MSG_AUTH\n");
//		gui_401login_open(bw, c, data.auth_realm);
	...
	case CONTENT_MSG_SSL:
		////kprintf("DEBUG: Processing CONTENT_MSG_SSL\n");
	...
	case CONTENT_MSG_REFRESH:
		////kprintf("DEBUG: Processing CONTENT_MSG_REFRESH\n");
		bw->refresh_interval = data.delay * 100;
		break;

	default:
		////kprintf("DEBUG: Unknown message type: %d\n", msg);
		assert(0);
	}
	
	////kprintf("DEBUG: browser_window_callback completed for msg %d\n", msg);
}
		...
		bool scroll_to_top)
{
	struct box *pos;
	int x, y;
LOG(("browser_window_update"));
	if (!bw->current_content)
		return;
LOG(("browser_window_update: bw %p, content %p, type %d",
		bw, bw->current_content, bw->current_content->type));
...
LOG(("browser_window_update: extent set"));
	if (scroll_to_top)
...
LOG(("browser_window_update: before gui_window_redraw_window"));
